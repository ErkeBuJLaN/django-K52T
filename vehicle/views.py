...
# пайдаланушы-Клиентті, механикті немесе әкімшіні тексеру үшін функциялар
...
# Әкімшіге қатысты көріністерді іске қосу
...
def admin_approve_request_view(request):
    enquiry = models.Request.objects.all().filter(status='Күтілуде')
...
def admin_report_view(request):
    reports = models.Request.objects.all().filter(Q(status="Сәтті аяқталды") | Q(status="Қызмет қабылданды"))
    dict = {
        'reports': reports,
    }
...
# Клиенттерге қатысты көріністер басталады

@login_required(login_url='customerlogin')
@user_passes_test(is_customer)
def customer_dashboard_view(request):
    customer = models.Customer.objects.get(user_id=request.user.id)
    work_in_progress = models.Request.objects.all().filter(customer_id=customer.id, status='Жөнделу процессы').count()
    work_completed = models.Request.objects.all().filter(customer_id=customer.id).filter(
        Q(status="Сәтті аяқталды")).count()
    new_request_made = models.Request.objects.all().filter(customer_id=customer.id).filter(
        Q(status="Күтілуде") | Q(status="Қабылданды")).count()
    bill = models.Request.objects.all().filter(customer_id=customer.id).filter(
        Q(status="Сәтті аяқталды") | Q(status="Қызмет қабылданды") | Q(status="Жөнделу процессы")).aggregate(
        Sum('cost'))
    print(bill)
    dict = {
...
def customer_view_request_view(request):
    customer = models.Customer.objects.get(user_id=request.user.id)
    enquiries = models.Request.objects.all().filter(customer_id=customer.id, status="Күтілуде")
...
def customer_view_approved_request_view(request):
    customer = models.Customer.objects.get(user_id=request.user.id)
    enquiries = models.Request.objects.all().filter(customer_id=customer.id).exclude(status='Күтілуде')
...
# Механикаға қатысты көріністерді іске қосу
...
def mechanic_dashboard_view(request):
    ...
    work_in_progress = models.Request.objects.all().filter(mechanic_id=mechanic.id, status='Жөнделу процессы').count()
    work_completed = models.Request.objects.all().filter(mechanic_id=mechanic.id, status='Сәтті аяқталды').count()
    new_work_assigned = models.Request.objects.all().filter(mechanic_id=mechanic.id, status='Қабылданды').count()
    dict = {
        'work_in_progress': work_in_progress,
        'work_completed': work_completed,
...
def mechanic_salary_view(request):
    mechanic = models.Mechanic.objects.get(user_id=request.user.id)
    workdone = models.Request.objects.all().filter(mechanic_id=mechanic.id).filter(Q(status="Сәтті аяқталды"))
...
# Жән басқа көріністрді іск қосу
def aboutus_view(request):
    return render(request, 'vehicle/front/about.html')


def service_view(request):
    return render(request, 'vehicle/front/service.html')


def service_1(request):
    return render(request, 'vehicle/front/service-1.html')


def service_2(request):
    return render(request, 'vehicle/front/service-2.html')


def service_3(request):
    return render(request, 'vehicle/front/service-3.html')


def service_4(request):
    return render(request, 'vehicle/front/service-4.html')


def service_5(request):
    return render(request, 'vehicle/front/service-5.html')


def service_6(request):
    return render(request, 'vehicle/front/service-6.html')


def faq(request):
    return render(request, 'vehicle/front/faq.html')


def accounts(request):
    return render(request, 'vehicle/accounts.html')


def projects(request):
    return render(request, 'vehicle/front/project-gallery.html')


def login(request):
    return render(request, 'vehicle/front/login.html')


def register(request):
    return render(request, 'vehicle/front/register.html')
...
def contactus_view(request):
    ...
    return render(request, 'vehicle/front/contact.html', {'form': sub})
